      ...
      <div className="flex-1 flex flex-col justify-between">
        <div>
          <div className="flex justify-between items-start mb-1">
            <h3 className="text-sm sm:text-base font-bold text-gray-800 leading-tight line-clamp-1">{product.name}</h3>
            <span className="text-sm sm:text-base font-black text-blue-600 ml-1 shrink-0">${product.price}</span>
          </div>
          <p className="text-[10px] sm:text-xs text-gray-500 leading-snug line-clamp-2">{product.description}</p>
          {(product.size || product.stock !== undefined) && (
            <div className="flex flex-wrap gap-1 mt-1">
              {product.size && (
                <span className="text-[9px] sm:text-xs font-bold px-2 py-0.5 bg-gray-100 text-gray-600 rounded-full border border-gray-200">
                  Size: {product.size}
                </span>
              )}
              {product.stock !== undefined && (
                <span className={`text-[9px] sm:text-xs font-bold px-2 py-0.5 rounded-full border ${product.stock > 0 ? 'bg-green-50 text-green-700 border-green-200' : 'bg-red-50 text-red-700 border-red-200'}`}>
        ...
        <Button
          onClick={onClick}
          className="mt-2 sm:mt-3 w-full gap-1 sm:gap-2 py-2 sm:py-2.5 bg-gray-900 text-white font-bold rounded-lg sm:rounded-xl hover:bg-blue-600 shadow-lg flex items-center justify-center text-[10px] sm:text-sm"
        >
          <ArrowRight size={14} />
          Buy Now
        </Button>
...
const ProductSection = ({ items, desktopHeightClass, emptyMessage, onBuy, children }) => (
  <section
    className={`bg-[#272727] rounded-2xl sm:rounded-3xl mx-3 sm:mx-4 my-3 sm:my-4
      p-4 sm:p-8 lg:p-12 overflow-hidden shadow-2xl
